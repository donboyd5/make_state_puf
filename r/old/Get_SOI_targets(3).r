...
keeptargs <- target_list$h2vname[1:5]
...
statepuf.rhs <- cc.sparse$enhanced.targets$df.rhs # retrieve vector with the constraint values
constraint.rhs <- target.rules$target # compute constraint rhs vector for the puf
...
statepuf.vs.targets <- target.rules.enhanced %>%
  ungroup %>%
  select(vname, calc.rule, constraint.num, target.num, feasible, vdesc, h2vname, agi_group) %>%
...
statepuf.vs.targets %>%
  filter(!is.na(apdiff)) %>%
  select(h2vname, agi_group, target.value, statepuf.value, apdiff, tol.group, tol.default, vdesc) %>%
  arrange(desc(apdiff)) %>%
...
overrides <- tribble(
  ...
  "e26270", "[1,2.5e+04)", 1,
  "e01000", "[1,2.5e+04)", 1,
  "e03240", "[1e+05,2e+05)", 1,
  "e03240_n", "[-Inf,1)", 1,
  "e19200", "[1e+06, Inf)", 1,
  "e02500_n", "[1e+06, Inf)", .25,
  "e01700_n",	"[1e+06, Inf)", .1
)
overrides
...
#******************************************************************************************************************
constraint.coefficients.sparse <- constraint.coefficients.sparse %>%
  group_by(i) %>%
  mutate(nzcc.raw=nzcc,
         constraint_scale_factor=50 / median(nzcc.raw),
         nzcc.scaled=nzcc * constraint_scale_factor) %>%
  ungroup
...
feasible.targets %>%
  ...
  mutate_at(vars(clb, target.value, cub, statepuf.value), ~ . / 1e6) %>%
...
violations <-  check %>%
  filter(violation!=0) %>% 
  left_join(feasible.targets %>% select(target.num, tol.group, pdiff, tol.default, tol, vname, vdesc, h2vname, agi_group)) %>%
