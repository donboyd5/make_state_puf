...
#****************************************************************************************************
#                ONETIME: Prepare initial NY weights for the puf by ratio adjustment ####
#****************************************************************************************************
# pufny <- puf %>%
...
saveRDS(pufny1, paste0(globals$tc.dir, "NY/", "pufny1.rds"))


#****************************************************************************************************
#                Get state puf with ratio-adjusted weights ####
#****************************************************************************************************
...
opts <- list("print_level" = 5,
             ...
             "max_iter"=500,
             "output_file" = "nytarget2_30_scaleopts_ma57.out")
...
# save if desired
# saveRDS(result, paste0(globals$tc.dir, "NY/targets459/result.rds"))
# saveRDS(targets.tol, paste0(globals$tc.dir, "NY/targets459/targets.tol.rds"))
...
check <- tibble(cnum=targets.tol$cnum,
  ...
  left_join(targets.tol %>% select(cnum, rhs, tol.group, pdiff, tol.default, tol, h2vname, agi_group, vdesc), by="cnum") %>%
  select(cnum, target, rhs, everything())
